...
//  version1
//
//  Created by Luzhen Qian on 4/10/16.
...
public class AutoCompleteTextField:UITextField {
    ...
    private func setupAutocompleteTable(view:UIView){
        ...
        autoCompleteTableHeight = 1000.0
    }
    ...
    func textFieldDidChange(){
        ...
        onTextChange(text!)
        if (text!.characters.count == 0){ autoCompleteStrings = nil }
        dispatch_async(dispatch_get_main_queue(), { () -> Void in
            self.autoCompleteTableView?.hidden =  self.hidesWhenEmpty! ? self.text!.characters.count < 3 : false
        })
    }
